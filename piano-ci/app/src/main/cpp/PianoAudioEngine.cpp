...
public:
    ...
    void stop() {
        ...
        allNotesOff(); AAudioStream_requestStop(stream_); AAudioStream_close(stream_); stream_ = nullptr;
    }
    ...
    void noteOff(int midi) {
        ...
        if (sustain_.load(std::memory_order_relaxed)) sustainHolds_[midi].store(true, std::memory_order_relaxed);
    }
    void setSustain(bool enabled) {
        sustain_.store(enabled, std::memory_order_relaxed);
        if (!enabled) for (int midi=0; midi<kMidiNotes; ++midi) if (!gates_[midi].load(std::memory_order_relaxed)) sustainHolds_[midi].store(false, std::memory_order_relaxed);
    }
    void allNotesOff() {
        sustain_.store(false, std::memory_order_relaxed);
        for (int midi=0;midi<kMidiNotes;++midi) { gates_[midi].store(false,std::memory_order_relaxed); sustainHolds_[midi].store(false,std::memory_order_relaxed); }
    }
private:
    ...
    aaudio_data_callback_result_t render(float* output, int32_t frames) {
        ...
        for (int frame=0;frame<frames;++frame) {
            ...
            for (int midi=21;midi<=108;++midi) {
                ...
                const bool sounding=keyDown||held; auto& voice=voices_[midi];
                if (sounding&&!voice.wasSounding) { voice.phase=0.0; voice.envelope=std::max(voice.envelope,0.015f); }
                if (sounding) voice.envelope=std::min(1.0f,voice.envelope+attackStep); else if (voice.envelope>0.0f) voice.envelope=std::max(0.0f,voice.envelope-releaseStep);
                voice.wasSounding=sounding; if (voice.envelope<=0.0001f) continue;
                ...
                const float vg=static_cast<float>(velocities_[midi].load(std::memory_order_relaxed))/127.0f;
                const double p=voice.phase;
                const float tone=static_cast<float>(std::sin(p)+0.31*std::sin(2.0*p+0.035)+0.13*std::sin(3.0*p+0.09));
                mix += tone*voice.envelope*vg; ++active; voice.phase += increment; if (voice.phase>=kTwoPi) voice.phase-=kTwoPi;
            }
            if (active>0) mix=std::tanh(mix*(0.52f/std::sqrt(static_cast<float>(active))));
            output[frame*2]=mix; output[frame*2+1]=mix;
        }
        return AAUDIO_CALLBACK_RESULT_CONTINUE;
    }
    AAudioStream* stream_=nullptr; int32_t sampleRate_=48000;
...
extern "C" JNIEXPORT jboolean JNICALL Java_com_pianostudio_alpha_PianoAudioEngine_nativeStart(JNIEnv*, jobject) { return gEngine.start()?JNI_TRUE:JNI_FALSE; }
extern "C" JNIEXPORT void JNICALL Java_com_pianostudio_alpha_PianoAudioEngine_nativeStop(JNIEnv*, jobject) { gEngine.stop(); }
extern "C" JNIEXPORT void JNICALL Java_com_pianostudio_alpha_PianoAudioEngine_nativeNoteOn(JNIEnv*, jobject, jint midi, jint velocity) { gEngine.noteOn(midi,velocity); }
extern "C" JNIEXPORT void JNICALL Java_com_pianostudio_alpha_PianoAudioEngine_nativeNoteOff(JNIEnv*, jobject, jint midi) { gEngine.noteOff(midi); }
extern "C" JNIEXPORT void JNICALL Java_com_pianostudio_alpha_PianoAudioEngine_nativeSetSustain(JNIEnv*, jobject, jboolean enabled) { gEngine.setSustain(enabled==JNI_TRUE); }
extern "C" JNIEXPORT void JNICALL Java_com_pianostudio_alpha_PianoAudioEngine_nativeAllNotesOff(JNIEnv*, jobject) { gEngine.allNotesOff(); }
